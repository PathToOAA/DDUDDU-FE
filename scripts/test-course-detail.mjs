...
const source=fs.readFileSync('src/api/courseDetail.ts','utf8').replace('import { resolveCourseTransit } from "./odsayApi";','const resolveCourseTransit = globalThis.__resolve;');
const js=ts.transpileModule(source,{compilerOptions:{module:ts.ModuleKind.ESNext,target:ts.ScriptTarget.ES2022}}).outputText;
const {loadCourseDetail,courseSummary}=await import('data:text/javascript;base64,'+Buffer.from(js).toString('base64'));
...
console.log('8 checks passed: saved route roundtrip and request deduplication');
